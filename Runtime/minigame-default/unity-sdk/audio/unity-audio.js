...
function jsAudioCreateCompressedSoundClip(audioData, ptr, length) {
    ...
    if (isSupportBufferURL) {
        const url = tj.createBufferURL(audioData);
        soundClip.url = url;
        WEBAudio.audioBufferLength += length;
    }
    else {
    ...
    _JS_Sound_Init() {
        try {
            if (tj && tj.createWebAudioContext) {
                WEBAudio.audioContext = tj.createWebAudioContext();
